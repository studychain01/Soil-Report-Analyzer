...
from agno.tools.log import logger 
import streamlit as st 

st.sidebar.title("API Key Configuration")

openai_api_key = st.sidebar.text_input("OpenAI API Key", type="password")
models_labs_api_key = st.sidebar.text_input("ModelsLabs API Key", type="password")
...
if openai_api_key and models_labs_api_key:
    agent = Agent(
    ...
    if st.button("Generate Music"):
            ...
            with st.spinner("Generating music..."):
                try:
                    ...
                    if music.audio and len(music.audio) > 0: 
                        ...
                        if not response.ok:
                            st.error("Failed {response.status_code}")
                            st.stop()
                        
                        content_type = response.headers.get("Content-Type", "")
                        if "audio" not in content_type:
                            st.error(f"Invalid content type: {content_type}")
                            st.write(f"Downloaded content was not audio file")
                            st.stop()
        ...
        st.sidebar.warning("Please enter your API keys in the sidebar to use the app.")